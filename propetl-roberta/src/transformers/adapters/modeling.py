...
class Adapter(nn.Module):
    ...
    def forward_with_mask(self, x, down_mask, up_mask, residual_input, output_gating=False):
        # down = self.adapter_down(x)
        # linear + non_linear
        down = self.adapter_down[1](
            F.linear(x, self.adapter_down[0].weight * down_mask.T, self.adapter_down[0].bias)
        )
...
class AdapterSubnet(nn.Module):
    def __init__(
        ...
        self.down_mask = nn.Parameter(torch.Tensor(
            torch.Size([input_size, output_size])
        ))
        self.up_mask = nn.Parameter(torch.Tensor(
    ...
    def forward(self):
        return GetSubnet.apply(self.down_mask.abs(), self.sparsity), GetSubnet.apply(self.up_mask.abs(), self.sparsity)
...
class PrefixSubnet(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.sparsity = sparsity
        self.adapters_mask = nn.Parameter(torch.Tensor(
            torch.Size([n_layers, prefix_length, 2 * input_size])
        ))
...
class LoraSubnet(nn.Module):
    ...
    def forward(self):
        return GetSubnet.apply(self.lora_A_mask.abs(), self.sparsity), GetSubnet.apply(self.lora_B_mask.abs(), self.sparsity)
