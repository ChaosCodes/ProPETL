...
from .loading import AdapterFusionLoader, AdapterLoader, PredictionHeadLoader, WeightsLoader, MaskLoader
from .lora import LoRALayer, LoRA
from .modeling import Adapter, GLOWCouplingBlock, NICECouplingBlock, ParallelAdapter
...
class ModelAdaptersMixin(PushAdapterToHubMixin, ABC):
    ...
    def train_adapter(self, adapter_setup: Union[list, AdapterCompositionBlock], train_embeddings=False, tasks=None):
        """Sets the model into mode for training the given adapters."""
        self.train()
        self.freeze_model(True)
        adapter_setup = parse_composition(adapter_setup)
        self.apply_to_adapter_layers(lambda i, layer: layer.enable_adapters(adapter_setup, True, False, tasks=tasks))
    ...
    def add_shared_adapter(self, adapter_name: str, config=None, overwrite_ok: bool = False, set_active: bool = False, tasks = None):
        """
        Adds a new adapter module of the specified type to the model.
    ...
    def _add_shared_adapter_weights(self, adapter_name: str, adapter_config=None, tasks=None):
        ...
        if hasattr(adapter_config, 'is_parallel'):
            ...
            shared_adapter = adapter_class(
                ...
                down_sample=int(self.config.hidden_size // adapter_config["reduction_factor"]),
                config=adapter_config,
            )
            shared_adapter.train(self.training)

        self.apply_to_adapter_layers(lambda i, layer: layer.add_shared_adapter(adapter_name,
                                                                               i,
                                                                               adapter_config['sparsity'],
                                                                               shared_adapter,
                                                                               tasks=tasks
                                    ))
        # PHM Layer
...
class ModelWithHeadsAdaptersMixin(ModelAdaptersMixin):
    ...
    def add_shared_adapter(self, adapter_name: str, config=None, overwrite_ok: bool = False, set_active: bool = False, tasks = None):
        ...
        """
        if self.base_model is self:
            super().add_shared_adapter(adapter_name, config, overwrite_ok=overwrite_ok, set_active=set_active, tasks=tasks)
        else:
            self.base_model.add_shared_adapter(adapter_name, config, overwrite_ok=overwrite_ok, set_active=set_active, tasks=tasks)
