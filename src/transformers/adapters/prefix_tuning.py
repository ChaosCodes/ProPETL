...
from .modeling import Activation_Function_Class, PrefixSubnet
...
class SharePrefixTuning(nn.Module, ModuleUtilsMixin):
    def __init__(
        ...
        self.adapters_mask = PrefixSubnet(self.n_layers,
    ...
    def eject(self):
        ...
        key_values = self.extend_prefix(key_values).unsqueeze(0).expand(1, -1, -1) # batch_size x prefix_length x n_layers*2*input_size
    ...
    def forward(self, batch_size):
        ...
        embs = self.wte(input_tokens)
        key_values = self.control_trans(embs)
        key_values = self.extend_prefix(key_values).expand(batch_size, -1, -1)  # batch_size x prefix_length x n_layers*2*input_size
        # import pdb; pdb.set_trace()
        key_values = key_values.view(
            batch_size, self.config.prefix_length, self.n_layers * 2, self.n_heads, self.n_embd_per_head
        )  # *2 for key and value
...
class PrefixTuningGroup(nn.ModuleDict):
    def __init__(self, module_configs, prefix_tuning_config):
        ...
        if prefix_tuning_config["flat"]:
            ...
            prefix_tuning_class = SharePrefixTuning if prefix_tuning_config.share_adapter else PrefixTuning
