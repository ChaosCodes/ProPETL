...
class T5Trainer(Trainer):
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.optimizer is None:
            ...
            mask_name = "mask_scores"
            optimizer_grouped_parameters = [
                {
                    "params": [p for n, p in self.model.named_parameters() if not any(nd in n for nd in no_decay) and mask_name not in n],
                    "weight_decay": self.args.weight_decay,
                },
                {
                    "params": [p for n, p in self.model.named_parameters() if any(nd in n for nd in no_decay) and mask_name not in n],
                    "weight_decay": 0.0,
                },
                {
                    "params": [p for n, p in self.model.named_parameters() if  mask_name in n],
                    "weight_decay": self.args.weight_decay,
                },
            ]
    ...
    def evaluate(self, eval_datasets: Optional[Dict[str, Dataset]] = None, is_test=False, ignore_keys=None) -> Dict[str, float]:
        """
        ...
        for eval_task, eval_dataset in eval_datasets.items():
            ...
            if is_test:
                tasks_metric = {"test/" + eval_task + "_" + k: v for k, v in output.metrics.items()}
            else: 
    ...
    def adjust_optim(self, n_iter):
        if n_iter < self.args.freeze_adapter_for_steps:
            ...
            return
        if self.args.mask_lr_ratio != 1:
            self.optimizer.param_groups[-1]['lr'] = self.optimizer.param_groups[0]['lr'] * self.args.mask_lr_ratio
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        if self.args.local_rank != -1:
            model = torch.nn.parallel.DistributedDataParallel(
                ...
                find_unused_parameters= True if self.args.train_adapters else False
            )
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    wandb.log({"mask_lr": self.optimizer.param_groups[-1]['lr']}, step=self.state.global_step)
                    if self.state.global_step >= self.args.freeze_adapter_for_steps:
